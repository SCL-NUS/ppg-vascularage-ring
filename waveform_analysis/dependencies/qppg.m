...
%
% 15 march 2022 by Gizem Yilmaz
% Added condition for SR to decide for Oura vs Pleth
...
EYE_CLS = 0.4;         % /* eye-closing period is set to 0.34 sec (340 ms) for PPG */ 
% LPERIOD  = sps*8;       % /* learning period is the first LPERIOD samples */
D=1000/sps; 
LPERIOD  = sps*D;       % added by GYD @ 14.03.2
SLPW = 0.2;            % /* Slope width (170ms) for PPG */                        
NDP = 3;              % /* adjust threshold if no pulse found in NDP seconds */
...
while t <= to
	...
	if (temp > T1)    % /* found a possible ABP pulse near t */ 
        ...
        if maxd>mind+10
            ...
            % (oftpq)
            valley_v = round(tpq);
            for valley_i=round(max(2,tpq-round((oftpq-0.05)*fs))):round(min(tpq+round(0.05*fs),length(data)-1))
